...
class Image_Toolbox {
	...
	function _loadFile($filename) {
		if (file_exists($filename)) {
			...
			$filedata['width'] = $info[0]+10;
			$filedata['height'] = $info[1]+10;
		...
		/**
	     * Loads a image file with border
	     *
	     * @access private
	     * @param string $filename imagefile to load
	     * @return array associative array with the loaded filedata
	     */
		function _loadFile_withborder($filename) {
		global $border;
		global $bgc_img;
		
			if (file_exists($filename)) {
				$info = getimagesize($filename);
				$filedata['width'] = $info[0]+$border['w'];
				$filedata['height'] = $info[1]+$border['h'];
				($filedata['width'] >= $filedata['height']) ? ($filedata['bias'] = IMAGE_TOOLBOX_BIAS_HORIZONTAL) : ($filedata['bias'] = IMAGE_TOOLBOX_BIAS_VERTICAL);
	           	$filedata['aspectratio'] = $filedata['width'] / $filedata['height'];
				$filedata['type'] = $info[2];
				...
				switch ($filedata['type']) {
					case 1:
						...
						$functionname = $this->_imagecreatefunction;
						$filedata['resource'] = $functionname($filedata['width'], $filedata['height']);
						imagecopy($filedata['resource'], $dummy, 0, 0, 0, 0, 0.01*$filedata['width'], 0.01*$filedata['height']);
						imagedestroy($dummy);
						break;
						
					case 2:
	//					$filedata['resource'] = imagecreatefromjpeg($filename);
						$dummy = imagecreatefromjpeg($filename);
						$functionname = $this->_imagecreatefunction;
						$filedata['resource'] = $functionname($filedata['width'], $filedata['height']);
						$color = $this->_hexToPHPColor($bgc_img);
						/*
						$new_width = .3*$filedata['width'] ;
						$new_height = .3*$filedata['height'] ;
						$tmp_img = imagecreatetruecolor($new_width,$new_height);
						if(function_exists(imagecopyresampled)) {
							imagecopyresampled($tmp_img , $dummy, 0,0,0,0,$new_width,$new_height,$filedata['width'],$filedata['height']);
							} else {
							imagecopyresized($tmp_img , $dummy, 0,0,0,0,$new_width,$new_height,$filedata['width'],$filedata['height']);
						}
						$filedata['resource'] = $tmp_img ;
						unset($tmp_img );
						*/
						imagecopy($filedata['resource'], $dummy, $border['w']/2, $border['h']/2, 0, 0, $filedata['width'], $filedata['height']);
						imagefill($filedata['resource'],1,1,$color);
						unset($dummy);
	...
	function addImage() {
		...
		/**
		     * Generate a new image resource based on the given parameters with border
		     *
		     * Parameter:
		     * <i>string</i> <b>$file</b> imagefile to load<br>
		     * Or:<br>
			...
			function addImage4collage() {
				...
				if (($argc == 2 || $argc == 3) && is_int($args[0]) && is_int($args[1]) && (is_string($args[2]) || !isset($args[2]))) {
					...
					//bild aus datei laden. width und height original gr”sse
					$this->_img['operator'] = $this->_loadFile_withborder($args[0]);
